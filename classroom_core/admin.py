...
CourseEnrollmentRequest ,AssignmentFile ,AssignmentFileReview ,
StudentGroup ,UserProfile
)
...
class CourseEnrollmentRequestAdmin(admin.ModelAdmin ):
    list_display =[
    'student','course','status_badge',
    'reviewed_by_badge','created_at','reviewed_at'
    ]
    list_filter =[
    'status','course','reviewed_at','created_at'
    ]
    search_fields =[
    'student__username','course__title','motivation','review_comment'
    ]
  ...
  ('Временные метки',{
    ...
    def reviewed_by_badge(self ,obj ):
        if obj.reviewed_by :
            return format_html('<strong>{}</strong>',obj.reviewed_by.username )
        return '—'
    reviewed_by_badge.short_description ='Проверил'

@admin.register(AssignmentFile )
class AssignmentFileAdmin(admin.ModelAdmin ):
    list_display =[
    'file_preview','assignment','student',
    'uploaded_at','review_count'
    ]
    list_filter =[
    'assignment__course','uploaded_at','assignment'
    ]
    search_fields =[
    'file','description','student__username','assignment__title'
    ]
  ...
  ('Временные метки',{
    ...
    def file_preview(self ,obj ):
        ...
        return format_html('<strong>{}</strong>',filename )
    file_preview.short_description ='Файл'

    def review_count(self ,obj ):
        count =obj.reviews.count()
        return count
    review_count.short_description ='Проверок'

@admin.register(AssignmentFileReview )
class AssignmentFileReviewAdmin(admin.ModelAdmin ):
    list_display =[
    'file_preview','reviewer','status_badge',
    'points_display','reviewed_at'
    ]
    list_filter =[
    'status','file__assignment__course','reviewed_at','reviewer'
    ]
    search_fields =[
    'feedback','file__file','reviewer__username'
    ]
  ...
  ('Временные метки',{
    ...
    def file_preview(self ,obj ):
        filename =obj.file.file.name.split('/')[-1 ]
        return format_html('<strong>{}</strong>',filename )
    file_preview.short_description ='Файл'
    ...
    def points_display(self ,obj ):
        if obj.points is not None :
            return format_html('<strong>{}</strong>',obj.points )
        return '—'
    points_display.short_description ='Баллы'

@admin.register(StudentGroup )
class StudentGroupAdmin(admin.ModelAdmin ):
    list_display =['name','created_by','student_count','created_at']
...
class UserProfileAdmin(admin.ModelAdmin ):
    list_display =[
    'username_display','role_badge','department',
    'student_group_display','created_at'
    ]
    list_filter =['role','department','student_group']
    search_fields =[
    'user__username','user__email','user__first_name',
    'user__last_name','department','position'
    ]
    readonly_fields =['created_at','updated_at']
    raw_id_fields =['user','student_group']
